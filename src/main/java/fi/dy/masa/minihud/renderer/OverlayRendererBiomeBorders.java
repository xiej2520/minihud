...
public class OverlayRendererBiomeBorders extends OverlayRendererBase
{
    ...
    protected List<SubChunkPos> getSubChunksWithinRange(Entity cameraEntity, MinecraftClient mc)
    {
        ...
        // FIXME: getBottomY, getTopY in 1.17+
        int minCY = Math.max(0 >> 4, chunkY - viewDistanceVertical);
        int maxCY = Math.min((256 - 1) >> 4, chunkY + viewDistanceVertical);
    ...
    private void createBiomeMapping(World world)
    {
        ...
        {
            final Registry<Biome> registry = Registry.BIOME;
            int count = 0;
            int maxId = 0;
    ...
    protected static final class ColoredQuad {
        final Vec3i start;
        final int width;
        final int height;
        ...
        private ColoredQuad(Vec3i start, int width, int height, Direction side, int biomeId) {
            this.start = start;
            this.width = width;
            this.height = height;
            this.side = side;
