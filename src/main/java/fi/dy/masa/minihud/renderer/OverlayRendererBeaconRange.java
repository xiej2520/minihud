...
public class OverlayRendererBeaconRange extends OverlayRendererBase
{
    ...
    public static boolean checkNeedsUpdate(int chunkX, int chunkZ, List<CompoundTag> blockEntities)
    {
        synchronized (BEACON_POSITIONS)
        {
            if (RendererToggle.OVERLAY_BEACON_RANGE.getBooleanValue() &&
                Configs.Generic.BEACON_RANGE_AUTO_UPDATE.getBooleanValue())
            {
                ...
                for (CompoundTag be : blockEntities) {
                    checkNeedsUpdate(Registry.BLOCK_ENTITY_TYPE.get(new Identifier(be.getString("id"))));
                }
            }
        }
    ...
    public static void renderBeaconBoxForPlayerIfHoldingItem(Entity entity, MatrixStack matrixStack, MinecraftClient mc)
    {
        Item item = mc.player.getMainHandStack().getItem();

        if (item instanceof BlockItem && ((BlockItem) item).getBlock() == Blocks.BEACON)
        {
            renderBeaconBoxForPlayer(entity, matrixStack, mc);
        }

    }

    private static void renderBeaconBoxForPlayer(Entity entity, MatrixStack matrixStack, MinecraftClient mc)
    {
        ...
        int level = mc.player.isSneaking() ? Math.min(4, mc.player.inventory.selectedSlot + 1) : 4;
